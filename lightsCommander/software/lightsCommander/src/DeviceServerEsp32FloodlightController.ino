...
const char* deviceId = "5d2b0515d99dc40506220a8e";
...
String extractAndSendCommand(JsonData *rootObject, unsigned int *httpResponce){
    String ret ="{\"result\":{\"message\":\"an error occurred\",\"code\":400}}";
    *httpResponce = 400;
    uint16_t address = JSONDATA_ERRORNUMBER;
    unsigned long command = JSONDATA_ERRORNUMBER;
    unsigned long ResponceCode = 400, nodeResponceCode;
    JsonData *current;
    
    current = rootObject->getChild("command");
    if (current== NULL ) {
        ret ="{\"result\":{\"message\":\"command missing\",\"code\":400}}";
        *httpResponce = 400;
        return ret;
    }
    
    command = current->getValueAsULong();

     if (command >= COMMAND_COUNT) {
        ret ="{\"result\":{\"message\":\"invalid Command\",\"code\":400}}";
        *httpResponce = 400;
        return ret;
    }
    ...
    current = rootObject->getChild("addresses[]");//got keyval
    String nodeArrayResponceContent = "";
    
    unsigned int sendCount=0;
    if (current) {
        if (current->getValueType() == JSONTYPE_ARRAY) {
            current=current->getChildAt(0);//getting the array
            if (current)
                current =current->getChildAt(0);//getting the first number in the array
        }
    }
    if (current) {
        ResponceCode = 200;
        while(current != NULL) {
            address = current->getValueAsULong();
            Serial.println(address);
            nodeResponceCode = sendCommand(address, command)? 200 : 400;
            if (nodeResponceCode == 400)
                ResponceCode = 400;//must be 400 or the node server will not return the error to client??? don't know why
            if (sendCount > 0)
                nodeArrayResponceContent+=",";
            nodeArrayResponceContent+="{\"address\":" + String(address) +",\"code\":"+String(nodeResponceCode)+"}";
            current = current->getNext();
            sendCount++;
        }
        //ResponceCode = 200;//todo: remove
        if (sendCount < 1)
            ResponceCode = 400;
        if (ResponceCode == 200 ) {
            ret = "{\"result\":{\"message\":\"Command sent successfully to all nodes\",\"code\":"+String(ResponceCode)+"}}";
        }
        else {
            ret = "{\"result\":{\"message\":\"Unable to send command to one or more nodes\",\"code\":"+String(ResponceCode)+",";
            ret+=  "\"node_results\":["+nodeArrayResponceContent+"]}}";
        }
        
    } 

    *httpResponce = ResponceCode;
    return ret;
...
void handleCustom(WiFiClient* client, unsigned int postMethod, String callingUrl) {
    String strSend ="{\"result\":{\"message\":\"Yes here I am, ready for business\",\"code\":200}}";
    unsigned int httpResponce = 200;
    ...
    if (method == METHODS::METHOD_POST || method == METHODS::METHOD_DELETE)
    {
        ...
        if (parser.isValid()) {
            if (method == METHOD_POST) {
                ...
                strSend = extractAndSendCommand(parser.getRootObject(), &httpResponce);
    ...
    Serial.println(strSend);
    client->println(makeJsonResponseString(httpResponce, strSend));
